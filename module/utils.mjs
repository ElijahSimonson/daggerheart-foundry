...
export function formatLength(value, unit, options={}) {
    return _formatSystemUnits(value, unit, CONFIG.DAGGERHEART.movementUnits[unit], options);
}
...
export function formatNumber(value, { numerals, ordinal, words, ...options }={}) {
    if ( words && game.i18n.has(`DAGGERHEART.NUMBER.${value}`, false) ) return game.i18n.localize(`DAGGERHEART.NUMBER.${value}`);
...
function _formatNumberAsOrdinal(n, options={}) {
    ...
    return game.i18n.has(`DAGGERHEART.ORDINAL.${pr}`) ? game.i18n.format(`DAGGERHEART.ORDINAL.${pr}`, { number }) : number;
}
...
export function formatTime(value, unit, options={}) {
    ...
    const config = CONFIG.DAGGERHEART.timeUnits[unit];
    if ( config?.counted ) {
...
export function formatVolume(value, unit, options={}) {
    return _formatSystemUnits(value, unit, CONFIG.DAGGERHEART.volumeUnits[unit], options);
}
...
export function formatWeight(value, unit, options={}) {
    return _formatSystemUnits(value, unit, CONFIG.DAGGERHEART.weightUnits[unit], options);
}
...
export function prepareFormulaValue(model, keyPath, label, rollData) {
    ...
    try {
        ...
        if ( item.isEmbedded ) {
            const message = game.i18n.format("DAGGERHEART.FormulaMalformedError", { property, name: model.name ?? item.name });
...
export function replaceFormulaData(formula, data, { actor, item, missing="0", property }={}) {
    ...
    if ( (missingReferences.size > 0) && actor && property ) {
        ...
        const message = game.i18n.format("DAGGERHEART.FormulaMissingReferenceWarn", {
...
export function areKeysPressed(event, action) {
    ...
    return game.keybindings.get("daggerheart", action).some(b => {
...
/**
 * Log a console message with the "Daggerheart" prefix and styling.
...
export function log(message, { color="#6e0000", extras=[], level="log" }={}) {
    console[level](
        `%cDaggerheart | %c${message}`, `color: ${color}; font-variant: small-caps`, "color: revert", ...extras
    );
}
...
export function convertLength(value, from, to, { strict=true }={}) {
    ...
    return _convertSystemUnits(value, from, to, CONFIG.DAGGERHEART.movementUnits, { message, strict });
}
...
export function convertTime(value, from, { combat=false, strict=true, to }={}) {
    const base = value * (CONFIG.DAGGERHEART.timeUnits[from]?.conversion ?? 1);
    if ( !to ) {
        // Find unit with largest conversion value that can still display the value
        const unitOptions = Object.entries(CONFIG.DAGGERHEART.timeUnits)
            .reduce((arr, [key, v]) => {
    ...
    return { value: _convertSystemUnits(value, from, to, CONFIG.DAGGERHEART.timeUnits, { message, strict }), unit: to };
}
...
export function convertWeight(value, from, to, { strict=true }={}) {
    ...
    return _convertSystemUnits(value, from, to, CONFIG.DAGGERHEART.weightUnits, { message, strict });
}
...
export function defaultUnits(type) {
    return CONFIG.DAGGERHEART.defaultUnits[type]?.[
...
export async function preloadHandlebarsTemplates() {
    ...
    for ( const path of partials ) {
        ...
        paths[`daggerheart.${path.split("/").pop().replace(".hbs", "")}`] = path;
    }
...
function itemContext(context, options) {
    if ( arguments.length !== 2 ) throw new Error("#daggerheart-itemContext requires exactly one argument");
...
function concealSection(conceal, options) {
  ...
  <div class="unidentified-notice">
      <div>
          <strong>${game.i18n.localize("DAGGERHEART.Unidentified.Title")}</strong>
          <p>${game.i18n.localize("DAGGERHEART.Unidentified.Notice")}</p>
      </div>
  </div>`;
    return content;
}
...
/**
 * Register custom Handlebars helpers used by Daggerheart.
 */
export function registerHandlebarsHelpers() {
    Handlebars.registerHelper({
        getProperty: foundry.utils.getProperty,
        "daggerheart-concealSection": concealSection,
        "daggerheart-dataset": dataset,
        "daggerheart-formatCR": formatCR,
        "daggerheart-formatModifier": formatModifier,
        "daggerheart-groupedSelectOptions": groupedSelectOptions,
        "daggerheart-itemContext": itemContext,
        "daggerheart-linkForUuid": (uuid, options) => linkForUuid(uuid, options.hash),
        "daggerheart-numberFormat": (context, options) => formatNumber(context, options.hash),
        "daggerheart-numberParts": (context, options) => formatNumberParts(context, options.hash),
        "daggerheart-object": makeObject,
        "daggerheart-textFormat": formatText
    });
}
 ...
 * @param {object} config  The `CONFIG.DAGGERHEART` object to localize and sort. *Will be mutated.*
 */
 ...
 * @param {object} [options]
 * @param {ActorDH} [options.actor]  An optional reference actor.
 * @param {ItemDH} [options.item]    An optional reference item.
 * @returns {string|void}
 */
export function getHumanReadableAttributeLabel(attr, { actor, item }={}) {
    ...
    if ( item && attr.startsWith("activities.") ) {
        ...
        else if ( attr === "uses.spent" ) label = "DAGGERHEART.Uses";
    }
    ...
    // Derived fields.
    else if ( (attr === "attributes.ac.rating") ) label = "DAGGERHEART.ArmorRating";

    // Abilities.
    else if ( attr.startsWith("traits.") ) {
        const [, key] = attr.split(".");
        label = game.i18n.format("DAGGERHEART.TraitScoreL", { ability: CONFIG.DAGGERHEART.traits[key].label });
    }

    // Currency
    else if ( attr.startsWith("currency.") ) {
        const [, key] = attr.split(".");
        label = CONFIG.DAGGERHEART.currencies[key]?.label;
    }
