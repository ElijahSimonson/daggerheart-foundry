...
export function registerCustomEnrichers() {
  const stringNames = [
    "attack", "award", "check", "concentration", "damage", "heal", "healing", "item", "save",
  ];
...
async function enrichAttack(config, label, options) {
  ...
  for ( const value of config.values ) {
    if ( value in CONFIG.DAGGERHEART.attackModes ) config.attackMode = value;
  ...
  span.innerHTML = game.i18n.format(`EDITOR.DAGGERHEART.Inline.Attack${config._rules === "2014" ? "Long" : "Short"}`, {
    formula: createRollLink(displayFormula).outerHTML
  });

  if ( config.format === "extended" ) {
    const type = game.i18n.format(`DAGGERHEART.ATTACK.Formatted.${config._rules}`, {
      type: game.i18n.getListFormatter({ type: "disjunction" }).format(
        Array.from(activity?.validAttackTypes ?? []).map(t => CONFIG.DAGGERHEART.attackTypes[t]?.label)
      ),
      classification: CONFIG.DAGGERHEART.attackClassifications[activity?.attack.type.classification]?.label ?? ""
    }).trim();
    ...
    full.innerHTML = game.i18n.format("EDITOR.DAGGERHEART.Inline.AttackExtended", {
...
async function enrichAward(config, label, options) {
  ...
  block.classList.add("award-block", "daggerheart2");
  block.dataset.awardCommand = command;

  const entries = [];
  for ( let [key, amount] of Object.entries(parsed.currency) ) {
    const label = CONFIG.DAGGERHEART.currencies[key].label;
  ...
  if ( parsed.each ) award = game.i18n.format("EDITOR.DAGGERHEART.Inline.AwardEach", { award });

  block.innerHTML += `
    ${award}
    <a class="award-link" data-action="awardRequest">
      <i class="fa-solid fa-trophy"></i> ${label ?? game.i18n.localize("DAGGERHEART.Award.Action")}
    </a>
  `;
...
async function enrichCheck(config, label, options) {
  ...
  for ( let value of config.values ) {
    ...
    if ( slug in CONFIG.DAGGERHEART.enrichmentLookup.traits ) config.ability = slug;
    else if ( slug in CONFIG.DAGGERHEART.enrichmentLookup.tools ) config.tool.push(slug);
  ...
  if ( activity ) {
    ...
    for ( const associated of activity.check.associated ) {
      if ( associated in CONFIG.DAGGERHEART.tools ) config.tool.push(associated);
    }
    delete config.activity;
  }

  // TODO: Support "spellcasting" ability
  let abilityConfig = CONFIG.DAGGERHEART.enrichmentLookup.abilities[slugify(config.ability)];
  ...
  for ( let [index, skill] of config.skill.entries() ) {
    const skillConfig = CONFIG.DAGGERHEART.enrichmentLookup.skills[slugify(skill)];
    if ( skillConfig ) {
  ...
  for ( const tool of config.tool ) {
    const toolConfig = CONFIG.DAGGERHEART.tools[slugify(tool)];
    const toolUUID = CONFIG.DAGGERHEART.enrichmentLookup.tools[slugify(tool)];
  ...
  if ( complex ) {
    ...
    for ( const [ability, associated] of groups.entries() ) {
      ...
      if ( associated.length > 1 ) parts.push(
        game.i18n.format("EDITOR.DAGGERHEART.Inline.SpecificCheck", {
          ability: CONFIG.DAGGERHEART.enrichmentLookup.abilities[ability].label,
    ...
    if ( config.dc && !config.hideDC ) {
      label = game.i18n.format("EDITOR.DAGGERHEART.Inline.DC", { dc: config.dc, check: label });
    }
    label = game.i18n.format(`EDITOR.DAGGERHEART.Inline.Check${config.format === "long" ? "Long" : "Short"}`, { check: label });
...
function createCheckRequestButtons(dataset) {
  ...
  if ( (tools.length) <= 1 ) return [createRequestButton(dataset)];
  const baseDataset = { ...dataset };
  delete baseDataset.tool;
  return [
    ...tools.map(tool => createRequestButton({
      ability: CONFIG.DAGGERHEART.tools[tool]?.ability, ...baseDataset, format: "short", tool, type: "tool"
    }))
  ];
}
...
async function enrichDamage(configs, label, options) {
  ...
  for ( const c of configs ) {
    ...
    for ( const value of c.values ) {
      if ( value in CONFIG.DAGGERHEART.damageTypes ) c.type.push(value);
      else if ( value in CONFIG.DAGGERHEART.healingTypes ) c.type.push(value);
      else if ( value in CONFIG.DAGGERHEART.attackModes ) config.attackMode = value;
  ...
  for ( const [idx, formula] of config.formulas.entries() ) {
    ...
    const types = type?.split("|")
      .map(t => CONFIG.DAGGERHEART.damageTypes[t]?.label ?? CONFIG.DAGGERHEART.healingTypes[t]?.label)
      .filter(_ => _);
    ...
    parts.push(game.i18n.format(`EDITOR.DAGGERHEART.Inline.Damage${localizationType}`, localizationData));
  }
  ...
  if ( config.average && (parts.length === 2) ) {
    link.innerHTML = game.i18n.format("EDITOR.DAGGERHEART.Inline.DamageDouble", { first: parts[0], second: parts[1] });
  } else {
  ...
  if ( config.format === "extended" ) {
    ...
    span.innerHTML = game.i18n.format("EDITOR.DAGGERHEART.Inline.DamageExtended", { damage: link.outerHTML });
    return span;
  }
 ...
 * ```[[lookup @target.template.size activity=daggerheartactivity000]]```
 * becomes
 * ```html
 ...
 *      data-uuid="Compendium.daggerheart.rules.JournalEntry.w7eitkpD7QQTB6j0.JournalEntryPage.UWw13ISmMxDzmwbd"
...
async function enrichReference(config, label, options) {
  let key;
  let source;
  let type = Object.keys(config).find(k => k in CONFIG.DAGGERHEART.ruleTypes);
  if ( type ) {
    key = slugify(config[type]);
    const { references } = CONFIG.DAGGERHEART.ruleTypes[type] ?? {};
    source = foundry.utils.getProperty(CONFIG.DAGGERHEART, references)?.[key];
  } else if ( config.values.length ) {
    key = slugify(config.values.join(""));
    for ( const [t, { references }] of Object.entries(CONFIG.DAGGERHEART.ruleTypes) ) {
      source = foundry.utils.getProperty(CONFIG.DAGGERHEART, references)?.[key];
      if ( source ) {
        type = t;
        break;
  ...
  if ( (type === "condition") && (config.apply !== false) ) {
    ...
    apply.setAttribute("aria-label", game.i18n.localize("EDITOR.DAGGERHEART.Inline.ApplyStatus"));
...
export function createRollLabel(config) {
  const { label: ability, abbreviation } = CONFIG.DAGGERHEART.abilities[config.ability] ?? {};
  const toolUUID = CONFIG.DAGGERHEART.enrichmentLookup.tools[config.tool];
  ...
  switch ( config.type ) {
    ...
    case "tool":
      if ( ability && (skill || tool) ) {
        label = game.i18n.format("EDITOR.DAGGERHEART.Inline.SpecificCheck", { ability, type: skill ?? tool });
      } else {
        label = ability;
      }
      if ( config.passive ) {
        label = game.i18n.format(
          `EDITOR.DAGGERHEART.Inline.${showDC ? "DC" : ""}Passive${longSuffix}`, { dc: config.dc, check: label }
        );
      } else {
        if ( showDC ) label = game.i18n.format("EDITOR.DAGGERHEART.Inline.DC", { dc: config.dc, check: label });
        label = game.i18n.format(`EDITOR.DAGGERHEART.Inline.Check${longSuffix}`, { check: label });
      }
      break;
    case "concentration":
    case "save":
      if ( config.type === "save" ) label = ability;
      else label = `${game.i18n.localize("DAGGERHEART.Concentration")} ${ability ? `(${abbreviation})` : ""}`;
      if ( showDC ) label = game.i18n.format("EDITOR.DAGGERHEART.Inline.DC", { dc: config.dc, check: label });
      label = game.i18n.format(`EDITOR.DAGGERHEART.Inline.Save${longSuffix}`, { save: label });
      break;
    default:
      return "";
  }
...
function createRequestLink(label, dataset) {
  ...
  if ( game.user.isGM ) {
    ...
    gmLink.dataset.tooltip = "EDITOR.DAGGERHEART.Inline.RequestRoll";
...
async function rollAction(event) {
  ...
  const options = { event };
  if ( ability in CONFIG.DAGGERHEART.abilities ) options.ability = ability;
  ...
  if ( (action === "roll") || !game.user.isGM ) {
    ...
    try {
      ...
      if ( !actors.length ) {
        ui.notifications.warn("EDITOR.DAGGERHEART.Inline.Warning.NoActor", { localize: true });
        return;
      }
  ...
  // Roll request
    ...
    const chatData = {
      ...
      content: await foundry.applications.handlebars.renderTemplate(
        "systems/daggerheart/templates/chat/request-card.hbs", { buttons }
      ),
      flavor: game.i18n.localize("EDITOR.DAGGERHEART.Inline.RollRequest"),
...
async function rollAttack(event) {
  ...
  const messageConfig = {
    data: {
      ...
      flavor: game.i18n.localize("DAGGERHEART.AttackRoll"),
  ...
  if ( rolls?.length ) {
    Hooks.callAll("daggerheart.rollAttackV2", rolls, { subject: null, ammoUpdate: null });
    Hooks.callAll("daggerheart.postRollAttack", rolls, { subject: null });
  }
}
...
async function rollDamage(event) {
  ...
  const messageConfig = {
    ...
    data: {
      ...
      flavor: game.i18n.localize(`DAGGERHEART.${rollType === "healing" ? "Healing" : "Damage"}Roll`),
      speaker: ChatMessage.implementation.getSpeaker()
    }
  };
  ...
  Hooks.callAll("daggerheart.rollDamageV2", rolls);
}
...
async function _fetchActivity(uuid, scaling) {
  ...
  const item = activity.item.clone({ "flags.daggerheart.scaling": scaling }, { keepId: true });
...
async function useItem({ rollActivityUuid, rollActivityName, rollItemUuid, rollItemName, rollItemActor }={}) {
  ...
  if ( !item ) {
    ...
    if ( item && canvas.tokens.controlled.length ) ui.notifications.warn(
      game.i18n.format("MACRO.DHMissingTargetWarn", {
  ...
  if ( item ) {
    if ( rollActivityName ) {
      ...
      else ui.notifications.warn(game.i18n.format("EDITOR.DAGGERHEART.Inline.Warning.NoActivityOnItem", {
  ...
  else ui.notifications.warn(game.i18n.format("EDITOR.DAGGERHEART.Inline.Warning.NoItemOnActor", {
