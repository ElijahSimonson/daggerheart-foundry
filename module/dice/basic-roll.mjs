 ...
 * @property {boolean} [evaluate=true]          Should the rolls be evaluated? If set to `false`, then no chat message will be created regardless of message configuration
 ...
 * @property {Document} [subject]               Document that initiated the roll
 ...
 * @property {ChatMessageDH} [document]   Final created chat message document once process is completed.
 * @property {string} [rollMode]          The roll made to apply to this message from `CONFIG.Dice.rollModes`
...
export default class BasicRoll extends Roll {
   ...
   * @param {BasicRollDialogConfiguration} [dialog={}]    Configuration for the roll prompt
   * @param {BasicRollMessageConfiguration} [message={}]  Configuration for the message creation
   * @return {BasicRoll[]}
   */
  ...
  /**
   * State one of the standard rolling workflow, configuring the roll
   * @param {BasicRollProcessConfiguration} [config={}]   Configuration for the rolls
   * @param {BasicRollDialogConfiguration} [dialog={}]    Configuration for the roll prompt
   * @param {BasicRollMessageConfiguration} [message={}]  Configuration for the message creation
  ...
  static async buildConfigure(config={}, dialog={}, message={}){
    ...
    /**
     * A hook event that fires before a roll is performed. Multiple hooks may be called depending on the rolling method. (eg. `daggerheart.preRollSkill`, `daggerheart.preRollAbilityCheck`, `daggerheart.preRoll`).
     * Exact contents of the configuration object will also change based on the roll type, but the same objects will always be present.
     * @function daggerheart.preRoll
     * @memberof hookEvents
    ...
    for (const hookName of config.hookNames){
      if (Hooks.call(`daggerheart.preRoll${hookName.capitalize()}`, config, dialog, message) === false) return [];
      if (Hooks.call(`daggerheart.preRoll${hookName.capitalize()}V2`, config, dialog, message) === false) return [];
    }
    ...
    if (dialog.configure === false){
      rolls = config.rolls?.map((r, index) => {
        ...
        for (const hookName of config.hookNames) {
          Hooks.callAll(`daggerheart.postBuild${hookName.capitalize()}RollConfig`, config, r, index);
        }
    ...
    const rollType = foundry.utils.getProperty(message, "data.flags.daggerheart.roll.type");
     ...
     * Multiple hooks may be called depending on the rolling method (eg. `daggerheart.postSkillCheckRollConfiguration`, `daggerheart.postAbilityTestRollConfiguration` and `daggerheart.postRollConfiguration` for skill checks).
     * Exact contents of the configuration object will also change based on the roll type, but the same objects will always be present.
     * @function daggerheart.postRollConfiguration
     * @memberof hookEvents
     ...
     * @returns {boolean}                             Explicity return `false` to prevent rolls
     */
    for (const hookName of config.hookNames){
      const name = `daggerheart.post${hookName.capitalize()}RollConfiguration`;
   ...
   * @param {BasicRollProcessConfiguration} [config={}]   Configuration for the roll
   * @param {BasicRollMessageConfiguration} [message={}]  Configuration for the message
   */
  ...
  static async buildPost(rolls, config, message){
    ...
    if (messageId) foundry.utils.setProperty(message.data, "flags.daggerheart.originatingMessage", messageId);
  ...
  /**
   * Is the result of this roll a failure? Returns `undefined` if roll wasn't evaluated.
   * @type {boolean|void}
   */
  get isFailure(){
  ...
  /**
   * Is the result of this roll a success? Returns `undefined` if the roll wasn't evaluated.
   * @type {boolean|void}
   */
  get isSuccess(){
    if (!this._evaluated) return;
  ...
  /**
   * Transform a Roll instance into a Chat Message, displaying the roll result.
   ...
   * @param {boolean} [options.create=true]             Whether to automatically create the chat message, or only return the prepared chatData object
   * @returns {Promise<ChatMessage>|object}             A promise that resolves to the created ChatMessage document if create is true, or the Object of prepared chatData otherwise.
   */
  ...
  static replaceFormulaData(formula, data, options){
    //Looks for the pattern `$!!$` and replaces it with the just the value between the marks.
    //The bang has been added to ensure this is a deliberate shim from the system, not an unintentional usage that should show an error.
    return super.replaceFormulaData(formula, data, options).replaceAll(/\$"??!(.+?)!"?\$/g, "$1");
  }

  /*  Maximize/Minimize Methods */
  /**
   * Replaces all dice terms that have modifiers with their maximum/minimum values
   *
   ...
   *
   * @param {DiceTerm} die            DiceTerm to get the max/min value
   * @param {object} [preCalculateOptions={}] Additional options modify preCalculate functionality
   * @param {boolean} [preCalculateOptions.minimize=false]  Calculate the minimum value instead of maximum
   * @return {number|null}          Maximum/Minimum value that could be rolled as a Integer, or null if the modifiers could not be precalculated.
   */
  static preCalculateTerm(die, {minimize=false}={}) {
      ...
      for (const modifier of currentModifiers){
        ...
        if (amount > 0) {
          if ((command === "max" && minimize) || (command === "max" && !minimize)) continue;
