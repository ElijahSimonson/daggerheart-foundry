...
/**
 * Parse the provided rolls, splitting parts based on damage types and properties, taking flavor into account.
 ...
 * @returns {DamageRoll}
 */
export default function aggregateDamageRoll(rolls, {respectProperties}={}){
  const makeHash = (type, properties=[]) => [type, ...(respectProperties ? Array.from(properties).sort() : [])].join();

  //Splits rolls into new sets of terms based on damage type and properties
  const types = new Map();
...
/**
 * Split terms into groups based on operators. Addition & Subtraction will split groups, multiplacation & division will keep groups together.
 * These groups also contain information on contained types written in flavor and whether they are negative.
...
function chunkTerms(terms, type){
  ...
  const isValidType = t => ((t in CONFIG.DAGGERHEART.damageTypes)|| (t in CONFIG.DAGGERHEART.healingTypes));
  const chunks = [];
  let currentChunk;
  let negative = false;
