...
export default class D20Die extends Die{
  ...
  /**
   * Is the result of the roll a critical success? Returns `undefined` if roll isn't evaluated
   * @type {boolean|void}
   */
  get isCriticalSuccess(){
    if (!this.isValid || !this._evaluated) return;
    if (!Number.isNumeric(this.options.criticalSuccess)) return false;
    return this.total >= this.options.cricialSuccess;
  }
  ...
  /**
   * Apply a maximum or minimum value to this die.
   * @param {object} values
