...
/**
 * A type of Roll Specific to a d20 based check, reaction, or attack roll in the Daggerheart System.
 * This roll is used by GM's only.
 * @see {@link HopeFearRoll} Roll for players to use
 *
...
export default class D20Roll extends BasicRoll {
  ...
  /**
   * Advantage mode of a Daggerheart D20 Roll
   * @enum {number}
   */
  static ADV_MODE = {
    NORMAL: 0,
   ...
   * @param {BasicRollMessageConfiguration} message Configuration data that guides the roll message creation
   */
  ...
  /**
   * The primary dies used in this d20 roll
   * @type {D20Die|void}
   */
  get d20(){
   ...
   */
  set d20(die){
    if (!(die instanceof CONFIG.Dice.D20Dice)) throw new Error(
  ...
  /**
   * Is this roll a critical success? Returns undefined if roll isnt evaluated
   * @type {boolean|void}
   */
  get isCritical(){
  ...
  static _prepareMessageData(rolls, messageData){
    ...
    messageData.flavor ??= "";
    if (advantage) messageData.flavor += ` (${game.i18n.localize("DAGGERHEART.Advantage")})`;
    else if (disadvantage) messageData.flavor += ` (${game.i18n.localize("DAGGERHEART.Disadvantage")})`;
  }
  ...
  configureModifiers(){
    ...
    //Recompile the underlying formula
    this.resetFormula();
  ...
  #createD20Die(){
    ...
    this.terms[0] = new CONFIG.Dice.D20Dice({...data, number, faces});
  }
}
