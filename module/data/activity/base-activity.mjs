...
import FormulaField from "../fields/formula-fields.mjs"
...
import ConsumptionField from "./fields/consumption-targets-field.mjs"

const {ArrayField, BooleanField, DocumentIdField, FilePathField, IntegerSortField,SchemaField, StringField} = foundry.data.fields;
 ...
 *
 * @property {string} id                        Unique ID for the activity on an item
 ...
 * @property {ActivationField} activation       Activation time and conditions
 ...
 * @property {boolean} consumption.scaling.allowed  Can this activity can be activated at higher levels?
 * @property {string} consumption.scaling.max       Maximum number of scaling levels for this item
 * @property {ConsumptionTargetingData[]} consumption.target    Collection of consumption targets
...
export default class BaseActivityData extends foundry.abstract.DataModel {
    ...
    static LOCALIZATION_PREFIXES = [
        "DAGGERHEART.ACTIVITY", "DAGGERHEAT.ACTIVATION", "DAGGERHEART.CONSUMPTION",
        "DAGGERHEART.DURATION", "DAGGERHEART.RANGE", "DAGGERHEART.TARGET", "DAGGERHEART.USES"
    ];
    ...
    get activationLabels() {
        if ( !this.activation.type ) return null;
    ...
    get canConfigureScaling() {
        return this.consumption.scaling.allowed || (this.item.system.level > 0);
    }
     ...
     */
    get canScaleDamage() {
        return this.consumption.scaling.allowed;
    }
     ...
     * @type {string}
     */
    static INITIAL_ID = staticID("dhactivity");
    ...
    static createInitialActivity(source, { offset=0, ...options }={}) {
        ...
        foundry.utils.setProperty(source, "flags.daggerheart.persistSourceMigration", true);
    }
    ...
    static transformDamagePartData(source, [formula, type]) {
        ...
        if ( parsed && CONFIG.DAGGERHEART.dieSteps.includes(Number(parsed[2])) ) {
        ...
        if ( (scaling && (Number(scaling[2]) === data.denomination)) ) {
    ...
    static transformEffectsData(source, options) {
        return source.effects
            .filter(e => !e.transfer && (e.type !== "enchantment") && (e.flags?.daggerheart?.type !== "enchantment"))
            .map(e => ({ _id: e._id }));
    }
    ...
    static transformTargetData(source, options) {
        ...
        if ( source.system.target?.type in CONFIG.DAGGERHEART.areaTargetTypes ) foundry.utils.mergeObject(data, {
            template: {
    ...
    prepareFinalData(rollData) {
        ...
        for ( const target of this.consumption.targets ) {
            ...
            if ( !actor.items.get(target.target) ) {
                const message = game.i18n.format("DAGGERHEART.CONSUMPTION.Warning.MissingItem", {
        ...
        this.labels.damage = this.labels.damages = (config.rolls ?? []).map(part => {
            ...
            if ( types.length ) {
                label = `${formula} ${game.i18n.getListFormatter({ type: "conjunction" }).format(
                    types.map(p => CONFIG.DND5E.damageTypes[p]?.label ?? CONFIG.DAGGERHEART.healingTypes[p]?.label).filter(_ => _)
                )}`;
            }
        ...
        return {
            ...
            options: {
                ...
                properties: Array.from(this.item.system.properties ?? [])
                    .filter(p => CONFIG.DAGGERHEART.itemProperties[p]?.isPhysical)
            }
        };
    }
