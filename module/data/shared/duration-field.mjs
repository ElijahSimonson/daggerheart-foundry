...
export default class DurationField extends SchemaField {
    ...
    static prepareData(rollData, labels) {
        this.duration.scalar = this.duration.units in CONFIG.DAGGERHEART.scalarTimePeriods;
        if ( this.duration.scalar ) {
            prepareFormulaValue(this, "duration.value", "DAGGERHEART.DURATION.FIELDS.duration.value.label", rollData);
        } else this.duration.value = null;

        if ( labels && this.duration.units ) {
            let duration = CONFIG.DAGGERHEART.timePeriods[this.duration.units] ?? "";
            ...
            labels.concentrationDuration = this.properties?.has("concentration")
                ? game.i18n.format("DAGGERHEART.ConcentrationDuration", { duration }) : duration;
        }
