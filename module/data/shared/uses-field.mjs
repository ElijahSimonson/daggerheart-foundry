...
export default class UsesField extends SchemaField {
    ...
    static prepareData(rollData, labels) {
        prepareFormulaValue(this, "uses.max", "DAGGERHEART.USES.FIELDS.uses.max.label", rollData);
        ...
        for ( const recovery of this.uses.recovery ) {
            if ( recovery.period === "recharge" ) {
                ...
                if ( labels ) labels.recharge ??= `${game.i18n.localize("DAGGERHEART.Recharge")} [${
                    recovery.formula}${parseInt(recovery.formula) < 6 ? "+" : ""}]`;
            } else if ( recovery.period in CONFIG.DAGGERHEART.limitedUsePeriods ) {
                const config = CONFIG.DAGGERHEART.limitedUsePeriods[recovery.period];
    ...
    static get rechargeOptions() {
        return Array.fromRange(5, 2).reverse().map(min => ({
            value: min,
            label: game.i18n.format("DAGGERHEART.USES.Recovery.Recharge.Range", {
    ...
    static getStatblockLabel() {
        ...
        if ( recovery.period === "recharge" ) {
            ...
            return `${game.i18n.localize("DAGGERHEART.Recharge")} ${value === 6 ? "6" : `${value}–6`}`;
        }

        // Recharge after a Short or Long Rest
        if ( ["lr", "sr"].includes(recovery.period) && (this.uses.max === 1) ) {
            return game.i18n.localize(`DAGGERHEART.Recharge${recovery.period === "sr" ? "Short" : "Long"}`);
        }

        // X/Day
        const period = CONFIG.DAGGERHEART.limitedUsePeriods[recovery.period === "sr" ? "sr" : "day"]?.label ?? "";
        if ( !period ) return "";
        ...
        if ( profile.type === "recoverAll" ) updates.spent = 0;
            ...
            try {
                ...
                if ( ["day", "dawn", "dusk"].includes(profile.period)
                    && (game.settings.get("daggerheart", "restVariant") === "gritty") ) {
                    roll.alter(7, 0, { multiplyNumeric: true });
                }
                total = (await roll.evaluate()).total;
            } catch(err) {
                Hooks.onError("UsesField#recoverUses", err, {
                    msg: game.i18n.format("DAGGERHEART.ItemRecoveryFormulaWarning", {
    ...
    static async rollRecharge(config={}, dialog={}, message={}) {
        ...
        messageConfig.data.flavor = game.i18n.format("DAGGERHEART.ItemRechargeCheck", {
            name: this.name,
            result: game.i18n.localize(`DAGGERHEART.ItemRecharge${rolls[0].isSuccess ? "Success" : "Failure"}`)
        });
         ...
         * been made.
         * @function daggerheart.rollRecharge
         * @memberof hookEvents
         * @param {BasicRoll[]} rolls             The resulting rolls.
         * @param {object} data
         * @param {ItemDH|Activity} data.subject  Item or Activity for which the roll was performed.
         * @param {object} data.updates           Updates to be applied to the subject.
         * @returns {boolean}                     Explicitly return `false` to prevent updates from being performed.
         */
        if ( Hooks.call("daggerheart.rollRecharge", rolls, { subject: this, updates }) === false ) return rolls;
        if ( Hooks.call("daggerheart.rollRechargeV2", rolls, { subject: this, updates }) === false ) return rolls;
         ...
         * @function daggerheart.postRollRecharge
         * @memberof hookEvents
         * @param {BasicRoll[]} rolls     The resulting rolls.
         * @param {object} data
         * @param {ActorDH} data.subject  Item or Activity for which the roll was performed.
         */
        Hooks.callAll("daggerheart.postRollRecharge", rolls, { subject: this });
