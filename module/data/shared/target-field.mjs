...
export default class TargetField extends SchemaField {
    ...
    static prepareData(rollData, labels) {
        this.target.affects.scalar = this.target.affects.type
            && (CONFIG.DAGGERHEART.individualTargetTypes[this.target.affects.type]?.scalar !== false);
        if ( this.target.affects.scalar ) {
            prepareFormulaValue(this, "target.affects.count", "DAGGERHEART.TARGET.FIELDS.target.affects.count.label", rollData);
        ...
        if ( this.target.template.type ) {
            ...
            prepareFormulaValue(this, "target.template.count", "DAGGERHEART.TARGET.FIELDS.target.template.count.label", rollData);
            prepareFormulaValue(this, "target.template.size", "DAGGERHEART.TARGET.FIELDS.target.template.size.label", rollData);
            prepareFormulaValue(this, "target.template.width", "DAGGERHEART.TARGET.FIELDS.target.template.width.label", rollData);
            prepareFormulaValue(this, "target.template.height", "DAGGERHEART.TARGET.FIELDS.target.template.height.label", rollData);
        } else {
        ...
        const templateConfig = CONFIG.DAGGERHEART.areaTargetTypes[this.target.template.type];
        if ( templateConfig ) {
            const parts = [];
            if ( this.target.template.count > 1 ) parts.push(`${this.target.template.count} ×`);
            if ( this.target.template.units in CONFIG.DAGGERHEART.movementUnits ) {
        ...
        const affectsConfig = CONFIG.DAGGERHEART.individualTargetTypes[this.target.affects.type];
            ...
            sheet: affectsConfig?.counted ? game.i18n.format(
                `${affectsConfig.counted}.${this.target.affects.count ? pr.select(this.target.affects.count) : "other"}`, {
                    number: this.target.affects.count ? formatNumber(this.target.affects.count)
                        : game.i18n.localize(`DAGGERHEART.TARGET.Count.${this.target.template.type ? "Every" : "Any"}`)
                }
            ).trim().capitalize() : (affectsConfig?.label ?? ""),
            statblock: game.i18n.format(
                `${affectsConfig?.counted ?? "DAGGERHEART.TARGET.Type.Target.Counted"}.${pr.select(this.target.affects.count || 1)}`,
    ...
    static templateDimensions(type) {
        const sizes = CONFIG.DAGGERHEART.areaTargetTypes[type]?.sizes;
        const dimensions = { size: "DAGGERHEART.AreaOfEffect.Size.Label" };
        if ( sizes ) {
            dimensions.width = sizes.includes("width") && (sizes.includes("length") || sizes.includes("radius"));
            dimensions.height = sizes.includes("height");
            if ( sizes.includes("radius") ) dimensions.size = "DAGGERHEART.AreaOfEffect.Size.Radius";
            else if ( sizes.includes("length") ) dimensions.size = "DAGGERHEART.AreaOfEffect.Size.Length";
            else if ( sizes.includes("width") ) dimensions.size = "DAGGERHEART.AreaOfEffect.Size.Width";
            if ( sizes.includes("thickness") ) dimensions.width = "DAGGERHEART.AreaOfEffect.Size.Thickness";
            else if ( dimensions.width ) dimensions.width = "DAGGERHEART.AreaOfEffect.Size.Width";
            if ( dimensions.height ) dimensions.height = "DAGGERHEART.AreaOfEffect.Size.Height";
        }
        return dimensions;
    }
}