...
export default class CurrencyTemplate extends SystemDataModel {
    ...
    static defineSchema() {
        return {
            ...
            }), {initialKeys: CONFIG.DAGGERHEART.currencies, initialKeysOnly: true, label: "DAGGERHEART.Currency"})
        };
    }
     ...
     */
    get currencyWeight() {
        if ( !game.settings.get("daggerheart", "currencyWeight") ) return 0;
        const count = Object.values(this.currency).reduce((count, value) => count + value, 0);
        const currencyPerWeight = game.settings.get("daggerheart", "metricWeightUnits")
            ? CONFIG.DAGGERHEART.encumbrance.currencyPerWeight.metric
            : CONFIG.DAGGERHEART.encumbrance.currencyPerWeight.imperial;
