 ...
 */
export default class TokenDocumentDH extends SystemFlagsMixin(TokenDocument) {
  ...
  static getConsumedAttributes(data) {
    return CONFIG.DAGGERHEART.consumableResources;
  }
  ...
  static getTrackedAttributeChoices(attributes) {
    ...
    const i18n = {
      traits: game.i18n.localize("DAGGERHEART.TraitScorePl"),
    };
    for ( const entry of groups ) {
      const { value } = entry;
      if ( value.startsWith("traits.") ) entry.group = i18n.traits;
    }
    return groups;
  }
  ...
  getRingColors() {
    ...
    if ( this.hasStatusEffect(CONFIG.specialStatusEffects.DEFEATED) ) {
      colors.ring = CONFIG.DAGGERHEART.tokenRingColors.defeated;
    }
    return colors;
  }
  ...
  flashRing(type) {
    ...
    const color = CONFIG.DAGGERHEART.tokenRingColors[type];
    if ( !color ) return;
    const options = {};
    if ( type === "damage" ) {
  ...
  async _preCreate(data, options, user) {
    ...
    if ( (this.actor?.type === "npc") && !this.actorLink
      ...
      const autoRoll = game.settings.get("daggerheart", "autoRollNPCHP");
  ...
  _onDelete(options, userId) {
    ...
    const origin = this.actor?.getFlag("daggerheart", "summon.origin");
    // TODO: Replace with parseUuid once V11 support is dropped
    if ( origin ) daggerheart.registry.summons.untrack(origin.split(".Item.")[0], this.actor.uuid);
  }
}
