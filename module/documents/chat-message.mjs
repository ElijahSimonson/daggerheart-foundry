...
export default class ChatMessageDH extends ChatMessage {
   ...
   */
  get canApplyDamage() {
    const type = this.flags.daggerheart?.roll?.type;
  ...
  get canSelectTargets() {
    if ( this.flags.daggerheart?.roll?.type !== "attack" ) return false;
  ...
  get isRoll() {
    ...
    return super.isRoll && !this.flags.daggerheart?.rest;
  }
  ...
  get shouldDisplayChallenge() {
    ...
    switch ( game.settings.get("daggerheart", "challengeVisibility") ) {
  ...
  static migrateData(source) {
    ...
    if ( foundry.utils.hasProperty(source, "flags.daggerheart.itemData") ) {
      foundry.utils.setProperty(source, "flags.daggerheart.item.data", source.flags.daggerheart.itemData);
      delete source.flags.daggerheart.itemData;
    }
    if ( foundry.utils.hasProperty(source, "flags.daggerheart.use") ) {
      const use = source.flags.daggerheart.use;
      foundry.utils.setProperty(source, "flags.daggerheart.messageType", "usage");
      if ( use.type ) foundry.utils.setProperty(source, "flags.daggerheart.item.type", use.type);
      if ( use.itemId ) foundry.utils.setProperty(source, "flags.daggerheart.item.id", use.itemId);
      if ( use.itemUuid ) foundry.utils.setProperty(source, "flags.daggerheart.item.uuid", use.itemUuid);
    }
    return source;
  }
  ...
  prepareData() {
    super.prepareData();
    if ( !this.flags.daggerheart?.item?.data && this.flags.daggerheart?.item?.id ) {
      const itemData = this.getFlag("daggerheart", "use.consumed.deleted")?.find(i => i._id === this.flags.daggerheart.item.id);
      if ( itemData ) Object.defineProperty(this.flags.daggerheart.item, "data", { value: itemData });
    }
    daggerheart.registry.messages.track(this);
  }
  ...
  async renderHTML(options={}) {
    ...
    this._highlightFearHope(html);
    if ( game.settings.get("daggerheart", "autoCollapseItemCards") ) {
    ...
    /**
     * A hook event that fires after daggerheart-specific chat message modifications have completed.
     * @function daggerheart.renderChatMessage
     * @memberof hookEvents
     * @param {ChatMessageDH} message  Chat message being rendered.
     * @param {HTMLElement} html       HTML contents of the message.
     */
    Hooks.callAll("daggerheart.renderChatMessage", this, html);
  ...
  _collapseTrays(html) {
    let collapse;
    switch ( game.settings.get("daggerheart", "autoCollapseChatTrays") ) {
  ...
  _displayChatActionButtons(html) {
    const chatCard = html.querySelector(".daggerheart.chat-card, .daggerheart2.chat-card");
    if ( chatCard ) {
  ...
  _highlightCriticalSuccessFailure(html) {
    ...
    const displayAttackResult = game.user.isGM || (game.settings.get("daggerheart", "attackRollVisibility") !== "none");
    const forceSuccess = this.flags.daggerheart?.roll?.forceSuccess === true;
    ...
    for ( let [index, d20Roll] of this.rolls.entries() ) {
      ...
      d20Roll = daggerheart.dice.D20Roll.fromRoll(d20Roll);
      ...
      const canCrit = ["attack", "death"].includes(this.getFlag("daggerheart", "roll.type"));
      const isAttack = this.getFlag("daggerheart", "roll.type") === "attack";
  ...
  /**
   * Highlight chat card based on fear or hope of the roll.
   * @param {HTMLElement} html Rendered contents of the message
   * @protected
   *
   * TODO - Implement styling of chat messages for Hope & Fear
   */
  _highlightFearHope(html) {
    const totals = html.querySelectorAll(".dice-total");
  ...
  _enrichChatCard(html) {
    ...
    anchor.setAttribute("aria-label", game.i18n.localize("DAGGERHEART.AdditionalControls"));
    ...
    // SVG icons
    html.querySelectorAll("i.daggerheart-icon").forEach(el => {
      const icon = document.createElement("daggerheart-icon");
      icon.src = el.dataset.src;
      el.replaceWith(icon);
    });

    // Enriched roll flavor
    const roll = this.getFlag("daggerheart", "roll");
    ...
    if ( this.isContentVisible && item && roll ) {
      ...
      const subtitle = roll.type === "damage"
        ? isCritical
          ? game.i18n.localize("DAGGERHEART.CriticalHit")
          : activity?.damageFlavor ?? game.i18n.localize("DAGGERHEART.DamageRoll")
        : roll.type === "attack"
          ? (activity?.getActionLabel(roll.attackMode) ?? "")
          : (item.system.type?.label ?? game.i18n.localize(CONFIG.Item.typeLabels[item.type]));
      const flavor = document.createElement("div");
      flavor.classList.add("daggerheart2", "chat-card");
      flavor.innerHTML = `
  ...
  _enrichAttackTargets(html) {
    ...
    if ( !(attackRoll instanceof daggerheart.dice.D20Roll) ) return;

    const visibility = game.settings.get("daggerheart", "attackRollVisibility");
    const isVisible = game.user.isGM || (visibility !== "none");
    if ( !isVisible ) return;

    const targets = this.getFlag("daggerheart", "targets");
    if ( !targets?.length ) return;
    const tray = document.createElement("div");
    tray.classList.add("daggerheart2");
    tray.innerHTML = `
      <div class="card-tray targets-tray collapsible collapsed">
        <label class="roboto-upper">
          <i class="fas fa-bullseye" inert></i>
          <span>${game.i18n.localize("DAGGERHEART.TargetPl")}</span>
          <i class="fas fa-caret-down" inert></i>
        </label>
        <div class="collapsible-content">
          <ul class="daggerheart2 unlist evaluation wrapper"></ul>
        </div>
      </div>
    `;
  ...
  _enrichDamageTooltip(rolls, html) {
    ...
    const aggregatedRolls = CONFIG.DAGGERHEART.aggregateDamageDisplay ? aggregateDamageRolls(rolls) : rolls;
    let { formula, total, breakdown } = aggregatedRolls.reduce((obj, r) => {
      obj.formula.push(CONFIG.DAGGERHEART.aggregateDamageDisplay ? r.formula : ` + ${r.formula}`);
    ...
    const tooltipContents = breakdown.reduce((str, { type, total, constant, dice }) => {
      const config = CONFIG.DAGGERHEART.damageTypes[type] ?? CONFIG.DAGGERHEART.healingTypes[type];
      return `${str}
    ...
    const damageOnSave = this.getFlag("daggerheart", "roll.damageOnSave");
    if ( damageOnSave ) {
      const p = document.createElement("p");
      p.classList.add("supplement");
      p.innerHTML = `<strong>${game.i18n.format("DAGGERHEART.SAVE.OnSave")}</strong> ${
        game.i18n.localize(`DAGGERHEART.SAVE.FIELDS.damage.onSave.${damageOnSave.capitalize()}`)
      }`;
    ...
    if ( game.user.isGM ) {
      ...
      damageApplication.classList.add("daggerheart2");
  ...
  _enrichEnchantmentTooltip(html) {
    const enchantmentProfile = this.getFlag("daggerheart", "use.enchantmentProfile");
    if ( !enchantmentProfile ) return;

    // Ensure concentration is still being maintained
    const concentrationId = this.getFlag("daggerheart", "use.concentrationId");
    ...
    enchantmentApplication.classList.add("daggerheart2");
  ...
  _enrichSaveTooltip(html) {
    ...
    content.classList.add("daggerheart2", "chat-card");

    // If message has the `forceSuccess` flag, mark it as resisted
    if ( roll.forceSuccess ) content.insertAdjacentHTML("beforeend", `
      <p class="supplement">
        <strong>${game.i18n.localize("DAGGERHEART.ROLL.Status")}</strong>
        ${game.i18n.localize("DAGGERHEART.LegendaryResistance.Resisted")}
      </p>
    `);
      ...
      content.insertAdjacentHTML("beforeend", `
        <div class="card-buttons">
          <button type="button">
            ...
            ${game.i18n.localize("DAGGERHEART.LegendaryResistance.Action.Resist")}
          </button>
        </div>
      `);
  ...
  _enrichUsageEffects(html) {
    if ( this.getFlag("daggerheart", "messageType") !== "usage" ) return;
    const item = this.getAssociatedItem();
    const effects = this.getFlag("daggerheart", "use.effects")
    ...
    effectApplication.classList.add("daggerheart2");
  ...
  static addChatMessageContextOptions(html, options) {
    ...
    options.push(
      {
        name: game.i18n.localize("DAGGERHEART.ChatContextDamage"),
      ...
      {
        name: game.i18n.localize("DAGGERHEART.ChatContextHealing"),
      ...
      {
        name: game.i18n.localize("DAGGERHEART.ChatContextTempHP"),
      ...
      {
        name: game.i18n.localize("DAGGERHEART.ChatContextDoubleDamage"),
      ...
      {
        name: game.i18n.localize("DAGGERHEART.ChatContextHalfDamage"),
      ...
      {
        name: game.i18n.localize("DAGGERHEART.ChatContextSelectHit"),
      ...
      {
        name: game.i18n.localize("DAGGERHEART.ChatContextSelectMiss"),
        icon: '<i class="fas fa-bullseye"></i>',
        condition: canTarget,
  ...
  applyChatCardDamage(li, multiplier) {
    const damages = aggregateDamageRolls(this.rolls, { respectProperties: true }).map(roll => ({
      value: Math.max(0, roll.total) * (roll.options.type in CONFIG.DAGGERHEART.healingTypes ? -1 : 1),
      type: roll.options.type,
      properties: new Set(roll.options.properties ?? [])
    }));
  ...
  _onDelete(options, userId) {
    ...
    daggerheart.registry.messages.untrack(this);
  }
  ...
  getAssociatedActivity() {
    const activity = fromUuidSync(this.getFlag("daggerheart", "activity.uuid"), { strict: false });
    if ( activity ) return activity;
    return this.getAssociatedItem()?.system.activities?.get(this.getFlag("daggerheart", "activity.id"));
  }
   ...
   */
  getAssociatedItem() {
    const item = fromUuidSync(this.getFlag("daggerheart", "item.uuid"), { strict: false });
    if ( item ) return item;
    const actor = this.getAssociatedActor();
    if ( !actor ) return;
    const storedData = this.getFlag("daggerheart", "item.data") ?? this.getOriginatingMessage().getFlag("daggerheart", "item.data");
   ...
   * @returns {ChatMessageDH[]}
   */
  getAssociatedRolls(type) {
    return daggerheart.registry.messages.get(this.id, type);
  }
  ...
  getOriginatingMessage() {
    return game.messages.get(this.getFlag("daggerheart", "originatingMessage")) ?? this;
  }
}
