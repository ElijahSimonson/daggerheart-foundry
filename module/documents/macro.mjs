import {indexFromUuid} from "../utils.mjs";
 ...
 * @returns {Promise}
 */
export async function createDHMacro(dropData, slot){
  const macroData = {type: "script", scope: "actor"}
  switch (dropData.type){
    case "Activity":
      const activity = await fromUuid(dropData.uuid);
      if (!activity) {
        ui.notifications.warn("MACRO.DHUnownedWarn", {localize: true});
        return null;
      }
      foundry.utils.mergeObject(macroData, {
        name: `${activity.item.name}: ${activity.name}`,
        img: activity.img,
        command: `daggerheart.documents.macro.rollItem("${activity.item._source.name}", {activityName: "${activity._source.name}", event })`,
    ...
    case "Item":
      ...
      foundry.utils.mergeObject(macroData, {
        name: itemData.name,
        img: itemData.img,
        command: `daggerheart.documents.macro.rollItem("${itemData._source.name}" , {event})`,
        flags: {"daggerheart.itemMacro": true}
      })
      break;
    case "ActiveEffect":
      const effectData = await ActiveEffect.implemntation.fromDropData(dropData);
      if (!effectData) {
        ui.notifications.warn("MACRO.DHUnownedWarn", {localize: true});
        return null;
      }
      foundry.utils.mergeObject(macroData, {
        ...
        img: effectData.img,
        command: `daggerheart.documents.macro.toggleEffect("${effectData.name}")`,
        flags: {"daggerheart.effectMacro": true}
      })
      break;
      default:
        return;
...
function getMacroTarget(name, documentType) {
  ...
  if ( !actor ) {
    ui.notifications.warn("MACRO.DHNoActorSelected", {localize: true});
    return null;
  }
  ...
  if ( documents.length === 0 ) {
    ui.notifications.warn(game.i18n.format("MACRO.DHMissingTargetWarn", { actor: actor.name, type, name }));
    return null;
  }
  if ( documents.length > 1 ) {
    ui.notifications.warn(game.i18n.format("MACRO.DHMultipleTargetsWarn", { actor: actor.name, type, name }));
  }
  return documents[0];
}
